...
const nodejsPath = devConf.gulp.nodejsPath?devConf.gulp.nodejsPath:"../generate-dir";

let compilePugFile= devConf.gulp.compilePugFile?devConf.gulp.compilePugFile:["dev/output-html/**/*.pug"];
...
let watchCompilePug = devConf.gulp.watchCompilePug || ["dev/output-html/**/*.pug", "dev/layout-component/**/*.pug"];
...
gulp.task("compile:pug", () => {
     ...
     *
     * */
    let opt = {base: 'dev/output-html/'};
    const time = Date.now();
    gulp.src(compilePugFile,opt)
        //.pipe(changed("dist/html", {extension: '.html'}))
        .pipe(pug({pretty: true, locals: {url: assetsUrl,time}}).on("error", (err) => {
            console.log(err)
        }))
...
gulp.task('compile:pug-component', () => {
    let opt = {base: '/dev/output-html/**'};
    gulp.src("dev/layout-component/**/*.pug")
        .pipe(pug({pretty: true, locals: {url: assetsUrl}}).on("error", (err) => {
            console.log(err)
        }))
        .pipe(gulp.dest("dist/html/layout-component"))
        .on("finish", () => {
            browserReload()
        })
});
...
gulp.task("compile:sass", (event) => {
    ...
    //.pipe(changed("dist/css")) disabled changed sass需要全部编译..
        ...
        .pipe(gulpif(extBase64File,base64({
            ...
            maxImageSize: 18*1024, // bytes 18KB
        })))
 ...
 */
//外联css转内联
// require("./gulpTasks/gulp-inline-css")();
